...
> 状态：[ ] 未开始、[~] 进行中、[x] 已完成、[!] 阻塞/缺陷。每个阶段完成后必须在本节更新状态、已执行测试与缺陷；未通过测试不得标记完成。

### 目标与边界

- Humanize 接管群聊短期上下文，替代 AstrBot 原生 history 的注入与回写；先捕获会话和人格标识，再关闭该请求的原生 history 保存，避免双重上下文。
- 上下文窗口按 unified_msg_origin、conversation_id 和 agent_id 隔离。窗口为同一 OpenViking Session 的有界投影，不新增第二套 SQLite 聊天记忆。
- 默认保留最多 40 个逻辑上下文条目；完成一个完整轮次后，条目数达到 40 或注入 token 超预算时，按完整轮次裁剪至不超过 20 个条目。
- 最近 10 个条目保持原始结构；第 11 至第 20 个条目仅折叠长文本、大工具结果和旧图片。当前用户消息永远完整保留。
- context_ref 在提示词和工具参数中必须是短、稳定、不可猜测的 ID，例如 ctx-7F3K9M2Q；内部 URI、完整 hash、数据库主键和文件路径不得注入模型或 WebUI。短 ID 映射由当前作用域的索引校验。

### 与现有记忆的融合

- 一个完成轮次只生成一份 canonical turn record：上下文窗口、OpenViking Session 和既有长期记忆任务通过同一个 turn_ref 关联，不重复保存两套普通聊天正文。
- 短期上下文由本计划的窗口提供；现有 L0/L1 Session fallback 仅在窗口不可用时降级使用，正常路径不得与窗口重复注入。
- 既有语义长期记忆、显式“记住”信号、黑话、回复样例和后台提取继续独立工作；它们作为相关事实补充注入，不等待窗口达到 40 条，也不依赖压缩总结。
- Session 摘要不再按每轮生成。压缩时才维护一个滚动 L1 总结；L2 保留被折叠内容的受控引用。长期记忆提取失败、图片缓存失败或压缩失败均不得阻断聊天。

### 运行流程

1. **请求前加载**：在 on_llm_request 读取当前 scope 的窗口，构造 req.contexts；保留系统提示词、当前输入、相关长期记忆和现有工具集，不让 AstrBot 原 history 混入。
2. **完成轮次落盘**：在 agent 完成后从 run_context.messages 提取用户、assistant、tool call、tool result 与最终可见回复，原子写入 canonical turn 和窗口索引。失败/中断轮次标记状态，不伪造 assistant 内容。
3. **工具原子性**：一次 assistant tool-call 链及其所有 result 属于一个逻辑条目；热窗口保留原生 assistant/tool 顺序，冷窗口只保留结构化摘要和 L2 ref，绝不留下孤立 tool message。
4. **图片一次解析**：图片首次发送时仍由 AstrBot 以原图多模态传给当前模型。该同一轮输出增加受限、不可见的 ImageCache（简短描述、OCR、关键对象），插件解析并剥离后缓存；不发起第二次图片分析请求。之后只注入缓存转述，不重复注入原图；无 vision 或缓存失败时写入 unavailable。工具产出的图片遵循同一规则。
5. **上下文压缩**：在完整轮次落盘后计算条目数和 provider 可用 token 预算。触发时选择最旧的完整轮次，保留最多 20 条；被淘汰范围与旧滚动总结合并为一个新的 L1 总结。先原子写入确定性简述和 L2 refs，再异步用 Provider 优化；优化失败保留确定性版本。
6. **L2 查询**：提供受控 read_context(ref) 工具，只允许读取当前 scope、会话和人格的内容，限制返回条数、字符数、调用次数与超时。返回数据标为不可信历史，不得当作系统指令。默认不保留旧原图；L2 只返回图片转述，细节不足时明确说明不可确认。
...
实现每个 C 阶段时先添加或更新对应测试，再运行定向测试；完成 C07 前必须主动执行完整验证，不以代码审阅替代测试。
...
~~~bash
uv run pytest -q tests/test_context.py tests/test_memory.py
uv run pytest -q
...
- 本节群聊上下文计划需要部署时，唯一部署入口仍是 bash scripts/deploy_hotfix.sh；先执行 --dry-run，再执行实际热修，不得改回手工 SSH/SCP 部署。
- 仅在本计划实现完成、CT01-CT08 已通过且远端健康检查成功后，使用**服务器上已运行的 AstrBot**和既有平台配置，向 QQ 3273963933 发送一次完成通知。
- 通知只包含部署结果与简短验收状态，例如“Humanize 群聊上下文更新已部署，验收通过。”；不得发送密码、token、聊天正文、内部路径、完整 ref 或异常堆栈。
- 发送前先确认目标 QQ 平台和现有 AstrBot 的受控发送路径可用；不读取或复制平台密钥，不新增机器人账号、不绕过现有权限。
- QQ 通知失败不影响已成功部署的判定；记录 CT09 为 [!] 并在最终汇报说明，禁止自动重复发送或刷屏。
