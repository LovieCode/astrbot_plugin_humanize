...
5. `Reply` 时 `<Reply>` 必须有有效文本；`No Reply` 时 `<Reply>` 必须为空。
6. `<UnknownTerm>` 的字段、数量、长度和置信度范围必须满足 schema。
7. 不允许 DTD、实体声明、外部实体或超出长度限制的 XML。
...
1. 开启 XML 严格模式时禁用最终回复流式下发，等待完整响应校验后发送 `<Reply>`。
...
6. 只有完整解析成功后，才处理 `<Action>`、`<UnknownTerms>` 和 `<Reply>`。
...
- 实现 `Reply` 的标签剥离与可见文本发送。
- 实现 `No Reply` 的结果清空和 `event.stop_event()`。
- 工具调用轮免检，但拦截工具轮附带的 Plain 文本；最终文本继续强校验。
- 严格模式禁用或缓冲流式输出。

验收：合法 `Reply` 只发送 `<Reply>` 内容；合法 `No Reply` 不发送任何消息；缺标签、半截 XML、自然语言裸输出和非法 Action 全部被拦截；工具调用正常执行。
...
- `<Msg>` 封装 → XML 最终响应 → Action 执行 → 只发送 `<Reply>` 文本。
