...
普通发言默认每条不超过 10 个可见字符，长度允许少量浮动、不可暴力截断；单次回复最多 5 条 Message（可配置）。最终输出协议校验失败时不做修复重试：直接阻断发送，向会话发一条系统通告（发送失败 + 中文原因），被拒绝的原始输出会带 `〔发送失败：{错误码}〕` 标注保存进托管历史，让后续轮次知道上次发送失败及其原因。代码、Markdown、日志、命令、教程和结构化数据不受日常分条规则限制。
