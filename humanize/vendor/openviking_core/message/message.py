...
from ..core.peer_id import normalize_peer_id
...
class Message:
    ...
    def __post_init__(self) -> None:
        """Normalize the optional peer identifier before it reaches a URI.

        Raises:
            ValueError: If ``peer_id`` is not a safe identifier segment.
        """
    ...
    def _part_to_dict(self, part: Part) -> dict:
        if isinstance(part, TextPart):
            ...
            if (
                part.tool_output_mime_type
                and part.tool_output_mime_type != "text/plain"
            ):
            ...
            if part.tool_output_externalization_error:
                d["tool_output_externalization_error"] = (
                    part.tool_output_externalization_error
                )
            if part.tool_output_group_id:
                d["tool_output_group_id"] = part.tool_output_group_id
            if part.tool_output_externalized_reason:
                d["tool_output_externalized_reason"] = (
                    part.tool_output_externalized_reason
                )
            if part.tool_output_group_original_chars is not None:
                d["tool_output_group_original_chars"] = (
                    part.tool_output_group_original_chars
                )
            if part.tool_output_group_budget_chars is not None:
                d["tool_output_group_budget_chars"] = (
                    part.tool_output_group_budget_chars
                )
            return d
        return {}
