...
class ToolPart:
    ...
    prompt_tokens: int | None = None  # 输入 Token
    completion_tokens: int | None = None  # 输出 Token
    tool_output_ref: str = ""
    tool_output_truncated: bool = False
    tool_output_original_chars: int | None = None
    tool_output_preview_chars: int | None = None
    tool_output_sha256: str = ""
    tool_output_storage_uri: str = ""
    tool_output_mime_type: str = "text/plain"
    tool_output_source_ref: str = ""
    tool_output_source_offset: int | None = None
    tool_output_source_limit: int | None = None
    tool_output_externalization_error: str = ""
    tool_output_group_id: str = ""
    tool_output_externalized_reason: str = ""
    tool_output_group_original_chars: int | None = None
    tool_output_group_budget_chars: int | None = None


Part = TextPart | ContextPart | ImagePart | ToolPart


def _parse_image_url_payload(data: dict[str, Any]) -> tuple[str, str | None]:
...
def part_from_dict(data: dict[str, Any]) -> Part:
    ...
    if part_type == "text":
        ...
        return ToolPart(
            ...
            tool_output_externalization_error=data.get(
                "tool_output_externalization_error", ""
            ),
            tool_output_group_id=data.get("tool_output_group_id", ""),
            tool_output_externalized_reason=data.get(
                "tool_output_externalized_reason", ""
            ),
            tool_output_group_original_chars=data.get(
