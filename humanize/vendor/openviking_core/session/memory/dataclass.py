...
class WikiLink(BaseModel):
    ...
    f: Annotated[int | None, WithJsonSchema({"type": "integer"})] = Field(
        ...,
        description="From page_id. Use the page_id from the item's 'page_id' field.",
    )
    t: Annotated[int | None, WithJsonSchema({"type": "integer"})] = Field(
...
class StoredLink(BaseModel):
    ...
    weight: float = 0.5
    match_text: str | None = None  # single word, must exist verbatim in conversation
    description: str = ""
    created_at: str = ""
...
class DeleteId(BaseModel):
    ...
    delete_page_id: Annotated[int | None, WithJsonSchema({"type": "integer"})] = Field(
...
class MemoryField(BaseModel):
    ...
    init_value: str | None = Field(None, description="Initial value for this field")
...
class MemoryTypeSchema(BaseModel):
    ...
    embedding_template: str | None = Field(None, description="Embedding text template")
    ...
    operation_mode: str = Field(
        "upsert",
        description="Operation mode: 'upsert' (default), 'add_only', or 'update_only'",
    )
    stage: str = Field(
        "user",
    ...
    overview_template: str | None = Field(
...
class MemoryData(BaseModel):
    ...
    memory_type: str = Field(..., description="Memory type name")
    uri: str | None = Field(None, description="Memory URI (for updates)")
    fields: dict[str, Any] = Field(
        default_factory=dict, description="Dynamic field data"
    )
    abstract: str | None = Field(None, description="L0 abstract")
    overview: str | None = Field(None, description="L1 overview")
    content: str | None = Field(None, description="L2 content")
    name: str | None = Field(None, description="Memory name")
    tags: list[str] = Field(default_factory=list, description="Tags")
    created_at: datetime | None = Field(None, description="Created time")
    updated_at: datetime | None = Field(None, description="Updated time")
...
class MemoryFile(BaseModel):
    ...
    def plain_content(self) -> str:
        from .utils.link_renderer import LinkRenderer

        return LinkRenderer.strip_links(self.content)

    @classmethod
    def from_parsed(
        cls, uri: str | None = None, parsed: dict[str, Any] = None
    ) -> "MemoryFile":
...
class ResolvedOperation(BaseModel):
    old_memory_file_content: MemoryFile | None = None
    memory_fields: dict
    memory_type: str  # The memory type (e.g., 'tools', 'skills', 'events')
    uris: list[str]
    page_id: int | None = None  # Temporary page_id for link resolution (not persisted)
    source: MemoryOperationSource | None = None
        ...
        if isinstance(data, dict):
            ...
            for field_name, value in data.items():
                if field_name in field_types:
                    data[field_name] = cls.value_fault_tolerance(
