...
from ..utils.line_numbers import (
    add_line_numbers,
    every_line_has_line_numbers,
    extract_start_line_number,
    strip_line_numbers,
)
from .base import StrPatch
    ...
    while left_index >= start_index or right_index <= end_index - search_len:
        ...
        if right_index <= end_index - search_len:
            if is_single_line:
                ...
                # Original multi-line logic
                original_chunk = "\n".join(
                    lines[right_index : right_index + search_len]
                )
                similarity = get_similarity(original_chunk, search_chunk)
                if similarity > best_score:
                    best_score = similarity
...
def validate_marker_sequencing(diff_content: str) -> dict[str, Any]:
    ...
    search_count = sum(1 for line in lines if re.match(SEARCH_PATTERN, line.strip()))
    sep_count = sum(1 for line in lines if line.strip() == SEP)
    replace_count = sum(1 for line in lines if line.strip() == REPLACE)
    ...
    for line in diff_content.split("\n"):
        ...
        if state["current"] == State.AFTER_SEPARATOR:
            if marker.startswith(":start_line:") and not line.strip().startswith(
                r"\:start_line:"
            ):
                return report_line_marker_in_replace_error(":start_line:")
            if marker.startswith(":end_line:") and not line.strip().startswith(
                r"\:end_line:"
            ):
    ...
    if state["current"] == State.START:
        ...
        expected = (
            "=======" if state["current"] == State.AFTER_SEARCH else ">>>>>>> REPLACE"
        )
        return {
            "success": False,
...
class DiffResult:
    ...
    error: str | None = None
    fail_parts: list[dict] | None = None
...
class MultiSearchReplaceDiffStrategy:
    ...
    def apply_diff(
        ...
        diff_content: str,
        _param_start_line: int | None = None,
        _param_end_line: int | None = None,
    ) -> DiffResult:
        """
        ...
        for replacement in replacements:
            ...
            if match_index == -1 or best_match_score < self.fuzzy_threshold:
                aggressive_search_content = strip_line_numbers(
                    search_content, aggressive=True
                )
                aggressive_replace_content = strip_line_numbers(
                    replace_content, aggressive=True
                )

                aggressive_search_lines = (
                    []
                    if aggressive_search_content == ""
                    else aggressive_search_content.split("\n")
                )
                ...
                if (
                    ...
                    replace_lines = (
                        [] if replace_content == "" else replace_content.split("\n")
                    )
                else:
            ...
            for i, line in enumerate(replace_lines):
                ...
                if relative_level >= 0:
                    final_indent = (
                        matched_indent + current_replace_indent[len(search_indent) :]
                    )
                else:
                    final_indent = matched_indent[
                        : max(0, len(matched_indent) + relative_level)
                    ]
        ...
        if applied_count == 0 and all_successful:
            return DiffResult(
                success=True, content=original_content, fail_parts=diff_results
            )
    ...
    def _parse_diff_blocks(self, diff_content: str) -> list[dict[str, Any]]:
        ...
        for block in blocks[1:]:
            ...
            replace_content = (
                after_sep.split(">>>>>>> REPLACE")[0].lstrip("\n").rstrip("\n")
            )
    ...
    for replacement in replacements:
        ...
        if start_line:
            ...
            if similarity >= strategy.fuzzy_threshold:
                ...
                search_end_index = min(
                    len(result_lines),
                    start_line + len(search_lines) + strategy.buffer_lines,
                )
