...
from ....core.namespace import uri_parts
...
class LinkRenderer:
    ...
    @staticmethod
    def _find_match_span(content: str, match_text: str) -> tuple[int, int] | None:
        ...
        linked_spans = [
            (m.start(), m.end())
            for m in LinkRenderer._RELATIVE_LINK_RE.finditer(content)
        ]
        ...
        for match in pattern.finditer(content):
            ...
            if LinkRenderer._is_ascii_word_char(
                left_char
            ) or LinkRenderer._is_ascii_word_char(right_char):
                continue
            return start, end
        return None
    ...
    def render_links(content: str, source_uri: str, links: list[dict]) -> str:
        ...
        """
        eligible = [link for link in links if link.get("match_text")]
        if not eligible:
            return content

        eligible.sort(key=lambda link: link.get("weight", 0), reverse=True)
    ...
    @staticmethod
    def relative_path(source_uri: str, target_uri: str) -> str | None:
