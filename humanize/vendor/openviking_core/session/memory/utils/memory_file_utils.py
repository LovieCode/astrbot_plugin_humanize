...
from ..dataclass import MemoryFile
from .link_renderer import LinkRenderer
from .messages import parse_memory_file_with_fields
from .uri import render_template

logger = logging.getLogger(__name__)

# Regex patterns for MEMORY_FIELDS HTML comment
_MEMORY_FIELDS_PATTERN = re.compile(
    r"\n\n<!--\s*MEMORY_FIELDS\s*\n(.*?)\n-->", re.DOTALL
)
_MEMORY_FIELDS_PATTERN_END = re.compile(
    r"<!--\s*MEMORY_FIELDS\s*\n(.*?)\n-->$", re.DOTALL
)
...
def memory_version_from_fields(
    fields: dict[str, Any] | None, *, default: int = 1
) -> int:
...
def next_memory_version(old_file: MemoryFile | None) -> int:
...
def _template_link_target(source_uri: str | None, target_uri: str) -> str:
    if source_uri and target_uri:
        return LinkRenderer.relative_path(str(source_uri), str(target_uri)) or str(
            target_uri
        )
...
class MemoryFileUtils:
    ...
    @staticmethod
    def read(raw_content: str, uri: str | None = None) -> MemoryFile:
    ...
    def truncate_content(
        content: str, max_chars: int = DEFAULT_TRUNCATE_MAX_CHARS
    ) -> str:
