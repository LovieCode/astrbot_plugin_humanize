...
from ..dataclass import MemoryTypeSchema
...
def validate_uri_template(memory_type: MemoryTypeSchema) -> bool:
    ...
    if memory_type.filename_template:
        ...
        template_vars = set(
            re.findall(r"\{\{\s*(\w+)\s*\}\}", memory_type.filename_template)
        )
...
def extract_uri_fields_from_flat_model(
    model: Any, schema: MemoryTypeSchema
) -> dict[str, Any]:
    """
