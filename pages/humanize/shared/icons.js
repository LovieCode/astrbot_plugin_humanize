/**
 * 共享线性 SVG 图标库（Lucide 风格，20px 画布、2px 描边、圆角端点）。
 * 所有页面通过 HZ.icon("name") 获取，禁止在页面里散落重复 SVG。
 */
(function (global) {
  const paths = {
    dashboard:
    ...
    check_simple: '<polyline points="20 6 9 17 4 12"/>',
    pause:
   ...
   * @param {number} [size] 可选尺寸（px），默认由 CSS 控制
  ...
  function icon(name, size) {
    ...
    return (
      `<svg${sizeAttr} viewBox="0 0 24 24" fill="none" stroke="currentColor"` +
