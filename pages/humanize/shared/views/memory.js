...
HZ.views["memory"] = { init: function () {
  ...
  function fillRecallControls() {
    ...
    // 召回 agent 下拉
    const agentSel = $("mem-recallAgent");
    if (agentSel) {
      agentSel.innerHTML = "";
      (state.agentOptions || [])
        .filter((a) => a.id !== "*")
  ...
  /* ================= 详情抽屉 ================= */
  const drawer = $("mem-drawer");
  const mask = $("mem-drawerMask");
  ...
  async function openDetail(id) {
    ...
    openDrawer();
    $("mem-drawerBody").classList.add("loading");
    try {
      const d = await HZ.api.get("memory-detail", { id });
      state.detail = d;
      renderDetail(d);
      $("mem-drawerBody").classList.remove("loading");
    } catch (e) {
      failToast(e);
      $("mem-drawerBody").classList.remove("loading");
      closeDrawer();
  ...
  /* ================= 召回测试 ================= */
  $("mem-recallBtn").addEventListener("click", runRecall);
  $("mem-recallQuery").addEventListener("keydown", (e) => {
    if (e.key === "Enter") runRecall();
  });

  async function runRecall() {
    const query = $("mem-recallQuery").value.trim();
    if (!query) {
